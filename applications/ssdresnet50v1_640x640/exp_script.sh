...
function measure_latency_gpu() {
    ...
    for i in $(seq 1 $numinstances); do
        local index=$(printf "%04d" $i)
        local container_name=pocket-client-${index}
        docker logs $container_name 2>&1 | grep "inference\_time\|be\_time\|fe\_time"
    done
...
function boottime() {
    ...
    for i in $(seq 1 $numinstances); do
        ...
        ### 1. Instantiate
        echo "[parse/boot/ssdresnet50v1/poc/$i/init] $(date +%s%6N)"
        ../../pocket/pocket \
            run \
                --measure-latency $rusage_logging_dir \
                -d \
                ...
                --env CONTAINER_ID=${container_name} \
                --workdir='/root/ssdresnet50v1' \
                -- bash -c "echo -n '[parse/boot/ssdresnet50v1/poc/$i/boot] ' && date +%s%6N && python3 app.pocket.boot.py" &
        interval=$(generate_rand_num 3)
        echo interval $interval
        sleep $interval
    done
...
function boottime_monolithic() {
    ...
    for i in $(seq 1 $numinstances); do
        ...
        echo "[parse/boot/ssdresnet50v1/mon/$i/init] $(date +%s%6N)"
        eval docker \
            run \
                -d "${GPUS}" \
                --name ${container_name} \
                ...
                --workdir=/root/ssdresnet50v1 \
                pocket-ssdresnet50v1-${DEVICE}-monolithic \
                bash -c \"echo -n \'[parse/boot/ssdresnet50v1/mon/$i/boot] \' \&\& date +%s%6N \&\& python3 app.monolithic.boot.py\"
...
function boottime_monolithic_cr() {
    ...
    for i in $(seq 1 $numinstances); do
        ...
        eval docker \
            run \
                ...
                --workdir=/root/ssdresnet50v1 \
                pocket-ssdresnet50v1-${DEVICE}-monolithic \
                bash -c \"echo -n \'[parse/boot/ssdresnet50v1/cr/$i/boot] \' \&\& date +%s%6N \&\& python3 app.monolithic.cr.boot.py\"
    ...
    for i in $(seq 1 $numinstances); do
        ...
        echo "[parse/boot/ssdresnet50v1/cr/$i/init] $(date +%s%6N)"
        docker \
            start \
