...
function measure_latency_monolithic() {
    ...
    # echo $running_time > "${rusage_logging_dir}"/talkingheads-monolithic-0000.latency
...
function boottime() {
    ...
    for i in $(seq 1 $numinstances); do
        ...
        ### 1. Instantiate
        echo "[parse/boot/talkingheads/poc/$i/init] $(date +%s%6N)"
        ../../pocket/pocket \
            run \
                --measure-latency $rusage_logging_dir \
                -d \
                ...
                --env CONTAINER_ID=${container_name} \
                --workdir='/root/talkingheads' \
                -- bash -c "echo -n '[parse/boot/talkingheads/poc/$i/boot] ' && date +%s%6N && python3 app.pocket.boot.py" &
        interval=$(generate_rand_num 3)
        echo interval $interval
        sleep $interval
    done
...
function boottime_monolithic() {
    ...
    for i in $(seq 1 $numinstances); do
        ...
        echo "[parse/boot/talkingheads/mon/$i/init] $(date +%s%6N)"
        eval docker \
            run \
                -d "${GPUS}" \
                --name ${container_name} \
                ...
                --workdir=/root/talkingheads \
                pocket-talkingheads-${DEVICE}-monolithic \
                bash -c \"echo -n \'[parse/boot/talkingheads/mon/$i/boot] \' \&\& date +%s%6N \&\& python3 app.monolithic.boot.py\"
...
function boottime_monolithic_cr() {
    ...
    for i in $(seq 1 $numinstances); do
        ...
        eval docker \
            run \
                ...
                --workdir=/root/talkingheads \
                pocket-talkingheads-${DEVICE}-monolithic \
                bash -c \"echo -n \'[parse/boot/talkingheads/cr/$i/boot] \' \&\& date +%s%6N \&\& python3 app.monolithic.cr.boot.py\"
    ...
    for i in $(seq 1 $numinstances); do
        ...
        echo "[parse/boot/talkingheads/cr/$i/init] $(date +%s%6N)"
        docker \
            start \
