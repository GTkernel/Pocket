...
import pocket_cgroups_helper as cghelper
...
def main(_argv):
    ...
    if FLAGS.comm == 'grpc':
        ...
        prof = cghelper.Profiler(os.getenv('BACKEND_UID'))
        prof.start_resource_measure(collect_backend=True)
        t1 = time.time()
        boxes, scores, classes, nums = yolo(img)
        t2 = time.time()
        prof.stop_resource_measure()
        (fe_mem, fe_cpu), (be_mem, be_cpu) = prof.get_resource_measure()
        logging.info(f'fe_cpu_time={fe_cpu}')
        logging.info(f'be_cpu_time={be_cpu}')
        logging.info('inference_time: {}'.format(t2 - t1))
...
if __name__ == '__main__':
    from ctypes import *
    libc = CDLL('libc.so.6')
    stack = c_char_p(b' ' * 8192)
    main_clone = CFUNCTYPE(c_int)(clone_main)
    stack_top = c_void_p(cast(stack, c_void_p).value + 8192)
    libc.clone(main_clone, stack_top, 0x08000000) #0x20000000) # 0x08000000==clone_newipc
