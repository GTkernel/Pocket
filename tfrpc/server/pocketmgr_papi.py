...
class TensorFlowServer:
    ...
    def _matmultest(client_id, N): ## test_code
        try:
            ...
            _matmultest_dict[client_id].append(mat_c)
    ...
    def tf_callable(client_id, typename, callable, args, _shmem=None):
        ...
        print('')
        print(f'measure start!!!!!!!!! {client_id}')
        papi.start()
        try:
            ...
            if type(ret) in (list, tuple):
                ...
                papi.stop()
                results = papi.read()
                os.makedirs(f'/data/poc/{NUM}', exist_ok=True)
                with open(f'/data/poc/{NUM}/event-{TIMESTAMP}-{EVENTSET}.log', 'a') as f:
