...
class PocketMessageChannel:
    ...
    def __init__(self):
        ...
        if PocketMessageChannel.__instance != None:
            ...
            if not IsolationControl.PRIVATEQUEUE:
                self.aux_gq = MessageQueue(PocketMessageChannel.aux_key)
    ...
    def hello_via_lq(self, message):
        ...
        if IsolationControl.PRIVATEQUEUE:
            ...
            self.aux_gq.send(args_json, block=True, type=CLIENT_TO_SERVER)
        raw_msg, _ = self.lq.receive(block=True, type=reply_type)
    ...
    #     self.aux_gq.send(args_json, type=CLIENT_TO_SERVER)
    #     raw_msg, _ = self.lq.receive(block=True, type=reply_type)
